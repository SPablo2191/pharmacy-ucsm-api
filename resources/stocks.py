...
class StocksAPI(Resource):
    def get(self):
        ...
        stocks = Stock.query.filter(
            or_(
                and_(Stock.product_id == product_id, Stock.status == True),
                and_(Stock.depot_id == depot_id, Stock.status == True),
            )
        ).all()
        return Response(
            stocks_schema.dumps(stocks), mimetype="application/json", status=200
        )
    ...
    def post(self):
        ...
        db.session.commit()
        return Response(
            stock_schema.dumps(new_stock), mimetype="application/json", status=200
        )
...
class StockAPI(Resource):
    def get(self):
        ...
        existing_stock = Stock.query.filter(
            or_(
                and_(Stock.product_id == product_id, Stock.depot_id == depot_id),
                Stock.id == stock_id,
            )
        ).all()
        ...
        if existing_stock:
            return Response(
                stocks_schema.dumps(existing_stock),
